    ...
    return (
        <div className="bg-slate-900 text-white min-h-screen">
            ...
            <div className="container mx-auto px-4">
                <div className="mb-8">
                    {/* Mock current plan - in production this would come from auth context */}
                    {(() => {
                        const currentPlan = 'Starter' // Change to 'Professional' or 'Enterprise' to test
                        const isFoundingProvider = true // Show founding provider banner by default
                        const datasetCount = 0
                        const maxDatasets = currentPlan === 'Starter' ? 1 : currentPlan === 'Professional' ? 5 : 999

                        const planColors: Record<string, { badge: string; text: string }> = {
                            Starter: { badge: 'bg-slate-700/80 text-slate-300', text: 'text-slate-300' },
                            Professional: { badge: 'bg-cyan-500/20 text-cyan-300 border border-cyan-500/30', text: 'text-cyan-300' },
                            Enterprise: { badge: 'bg-purple-500/20 text-purple-300 border border-purple-500/30', text: 'text-purple-300' },
                        }

                        const planPrices: Record<string, { price: string; period: string; renewal: string }> = {
                            Starter: { price: 'Free', period: 'Forever', renewal: 'N/A (Free)' },
                            Professional: { price: '$299', period: '/month', renewal: 'May 1, 2026' },
                            Enterprise: { price: '$999', period: '/month', renewal: 'May 1, 2026' },
                        }

                        const colors = planColors[currentPlan]
                        const pricing = planPrices[currentPlan]

                        return (
                            <>
                                {/* Founding Provider Banner */}
                                {isFoundingProvider && (
                                    <div className="mb-6 rounded-xl border border-emerald-500/40 bg-emerald-500/10 p-4">
                                        <div className="flex items-center gap-3">
                                            <span className="text-xl">⭐</span>
                                            <div>
                                                <div className="font-semibold text-emerald-200">Founding Provider</div>
                                                <div className="text-sm text-slate-300">Professional plan — Free until March 2027</div>
                                            </div>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="rounded-2xl border border-slate-700/60 bg-slate-800/60 p-6">
                                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                                            <div className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${colors.badge}`}>
                                                {currentPlan}
                                            </div>
                                            <div>
                                                <div className="text-lg font-semibold text-white">{currentPlan} Plan</div>
                                                <div className="text-sm text-slate-400">
                                                    {pricing.price}
                                                    {pricing.period !== 'Forever' && <span className="text-slate-500">{pricing.period}</span>}
                                                </div>
                                            </div>
                                            <div className="text-sm text-slate-400 border-l border-slate-600 pl-4">
                                                {currentPlan === 'Starter' ? (
                                                    <span>{pricing.renewal}</span>
                                                ) : (
                                                    <span>Next billing: {pricing.renewal}</span>
                                                )}
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
                                            <div className="flex-1 min-w-[200px]">
                                                <div className="text-xs text-slate-400 mb-2">
                                                    Datasets listed: {datasetCount} of {maxDatasets}
                                                </div>
                                                <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
                                                    <div 
                                                        className="h-full bg-cyan-500 rounded-full transition-all"
                                                        style={{ width: `${Math.min((datasetCount / maxDatasets) * 100, 100)}%` }}
                                                    />
                                                </div>
                                            </div>
                                            <button
                                                onClick={() => window.location.href = '/#provider-pricing'}
