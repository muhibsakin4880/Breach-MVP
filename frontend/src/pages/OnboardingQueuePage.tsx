...
const decisionClasses: Record<DecisionStatus, string> = {
    ...
    'Ready for signoff': 'border-emerald-500/20 bg-emerald-500/10 text-emerald-300',
    'Pilot approved': 'border-cyan-500/20 bg-cyan-500/10 text-cyan-300'
}

const loiStatusClasses: Record<LoiStatus, string> = {
    'Packet in preparation': 'text-slate-400',
    'Draft LOI shared': 'text-cyan-300',
    'LOI under review': 'text-amber-300',
    'Pilot scope agreed': 'text-emerald-300',
    'Pilot approved': 'text-cyan-300'
}
...
export default function OnboardingQueuePage() {
    ...
    const activePilotReviewCount = records.filter((record) =>
        ['Draft LOI shared', 'LOI under review', 'Pilot scope agreed'].includes(record.loiStatus)
    ).length
    const draftLoiCount = records.filter((record) => record.loiStatus === 'Draft LOI shared').length
    const scopedPilotCount = records.filter((record) => record.loiStatus === 'Pilot scope agreed').length
    ...
    const summaryCards = [
        ...
        {
            label: 'Active Pilot Reviews',
            value: activePilotReviewCount.toString(),
            detailLabel: 'LETTER STATUS',
            detailValue: `${draftLoiCount} draft letters · ${scopedPilotCount} scoped pilots`,
            progress: Math.round((activePilotReviewCount / records.length) * 100),
            tone: 'cyan' as const,
        ...
        },
        {
            ...
            detailValue: `${escalatedCount} packet(s) require policy or legal signoff`,
    ...
    return (
        <AdminLayout title="ORGANIZATION REVIEW QUEUE" subtitle="INTAKE, PILOT READINESS & APPROVAL STAGING">
                                            ...
                                            <tr key={record.id} className="align-top transition-colors duration-150 hover:bg-slate-800/20">
                                                ...
                                                <td className="px-5 py-4">
                                                    <div className="space-y-2">
                                                        <p className="text-[10px] leading-relaxed text-slate-300">{record.useCase}</p>
                                                ...
                                                <td className="px-5 py-4">
                                                    <div className="space-y-2">
                                                        <div className="flex flex-wrap gap-2">
                                                            ...
                                                            <span className={`inline-flex items-center rounded-md border px-2.5 py-1 text-[10px] font-semibold tracking-wide ${decisionClasses[record.decisionStatus]}`}>
                                                                {record.decisionStatus}
                                                            </span>
                                                        </div>
                                                        <p className={`text-[10px] font-medium ${loiStatusClasses[record.loiStatus]}`}>{record.loiStatus}</p>
                                                ...
                                                <td className="px-5 py-4">
                                                    <div className="space-y-2">
                                                        ...
                                                        <p className="max-w-[11rem] text-[9px] leading-relaxed text-slate-500">{record.nextAction}</p>
                                                    </div>
                                                </td>
                                            </tr>
                                        )
                    ...
                    <div className="col-span-4 overflow-hidden rounded-xl border border-slate-800/50 bg-slate-900/60 shadow-2xl shadow-black/30 backdrop-blur-xl">
                        <div className="border-b border-slate-800/60 px-5 py-4">
                            <div className="flex items-center justify-between">
                                <h2 className="text-[11px] font-semibold uppercase tracking-[0.1em] text-slate-300">Pilot Review Activity</h2>
                                    ...
                                    <div key={`${activity.message}-${idx}`} className={`px-5 py-3.5 ${getActivityStyle(activity.type)}`}>
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="min-w-0 flex-1">
                                                <span className="block text-[11px] leading-relaxed text-slate-400">{activity.message}</span>
