...
const decisionStatusLabels: Record<DecisionStatus, GovernanceDecisionStatus> = {
    'Awaiting first pass': 'Awaiting first pass',
    'Secondary review': 'Secondary review',
    'Ready for signoff': 'Ready for approval',
    'Pilot approved': 'Approved for evaluation'
}

const packetStatusLabels: Record<LoiStatus, ReviewPacketStatus> = {
    'Packet in preparation': 'Packet in preparation',
    'Draft LOI shared': 'Packet submitted',
    'LOI under review': 'Packet under review',
    'Pilot scope agreed': 'Evaluation scope aligned',
    'Pilot approved': 'Approved for evaluation'
}

export const getDecisionStatusLabel = (status: DecisionStatus): GovernanceDecisionStatus =>
    decisionStatusLabels[status]

export const getPacketStatusLabel = (status: LoiStatus): ReviewPacketStatus =>
    packetStatusLabels[status]

export const normalizeReviewCopy = (value: string) =>
    value
        .replace(/\bDraft LOI\b/g, 'review packet')
        .replace(/\bLOI\b/g, 'review packet')
        .replace(/\bdraft letters\b/g, 'submitted packets')
        .replace(/\bdraft letter\b/g, 'submitted packet')
        .replace(/\bPilot signoff\b/g, 'Final approval')
        .replace(/\bpilot signoff\b/g, 'final approval')
        .replace(/\bpilot approval\b/g, 'final approval')
        .replace(/\bpilot drafting\b/g, 'approval routing')
        .replace(/\bpilot letter\b/g, 'review packet')
        .replace(/\bpilot readiness\b/g, 'approval readiness')
        .replace(/\bPilot readiness\b/g, 'Approval readiness')
        .replace(/\bpilot review\b/g, 'governance review')
        .replace(/\bPilot review\b/g, 'Governance review')
        .replace(/\bpilot scope\b/g, 'evaluation scope')
        .replace(/\bPilot scope\b/g, 'Evaluation scope')
        .replace(/\bpilot request\b/g, 'protected evaluation request')
        .replace(/\bpilot program\b/g, 'evaluation program')
        .replace(/\bPilot program\b/g, 'Evaluation program')
        .replace(/\bscoped pilots\b/g, 'aligned evaluation scopes')
        .replace(/\bscoped pilot\b/g, 'aligned evaluation scope')
        .replace(/\bpilot path\b/g, 'approval path')
