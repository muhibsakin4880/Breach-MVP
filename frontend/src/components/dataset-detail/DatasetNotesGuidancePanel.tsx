            ...
            <div className="space-y-5">
                ...
                <div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)]">
                    <div className="rounded-2xl border border-slate-700 bg-slate-950/45 p-5">
                        ...
                        <div className="mt-4 space-y-3">
                            {dataset.access.instructions.map(item => (
                                <div key={item} className="rounded-xl border border-white/8 bg-slate-900/70 px-4 py-3 text-sm leading-6 text-slate-200">
                                    {item}
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="rounded-2xl border border-slate-700 bg-slate-950/45 p-5">
                        ...
                        <div className="mt-4 space-y-3">
                            <div className="rounded-xl border border-white/8 bg-slate-900/70 px-4 py-4">
                ...
                {requestStatus === 'REQUEST_APPROVED' ? (
                    <div className="rounded-2xl border border-green-500/30 bg-slate-900/60 p-5">
    ...
    return (
        <div className="rounded-2xl border border-slate-700 bg-slate-950/45 p-5">
            <div className="flex items-center gap-3">
                ...
                <span className={`rounded-full border px-3 py-1 text-xs ${subtitleClassName}`}>{subtitle}</span>
            </div>
            <div className="mt-4 space-y-3">
                {items.map(item => (
                    <div
                        key={item}
                        className="rounded-xl border border-slate-700 bg-slate-900/70 px-4 py-3 text-sm leading-6 text-slate-200"
                    >
                        {item}
                    </div>
                ))}
