...
export default function DealProgressTracker({
    ...
    const stageGridClass = compact
        ? 'grid-cols-[repeat(auto-fit,minmax(180px,1fr))]'
        : 'grid-cols-[repeat(auto-fit,minmax(170px,1fr))]'

    return (
        <section className="rounded-3xl border border-white/10 bg-[#0a1526]/88 p-5 shadow-[0_20px_60px_rgba(0,0,0,0.28)] backdrop-blur-xl sm:p-6">
            <div className="grid gap-4 [grid-template-columns:repeat(auto-fit,minmax(240px,1fr))]">
                <div className="min-w-0">
                    ...
                    <h2 className="mt-4 max-w-3xl text-2xl font-semibold leading-tight text-white">
                        {model.headline}
                    </h2>
                ...
                <div className="min-w-0 rounded-2xl border border-cyan-500/25 bg-cyan-500/8 px-4 py-3">
            ...
            <div className={`mt-6 grid gap-3 ${stageGridClass}`}>
                {model.stages.map((stage, index) => (
                    <article key={stage.key} className="min-w-0 rounded-2xl border border-white/8 bg-slate-950/45 p-4">
                        <div className="flex flex-wrap items-start gap-2">
                            <div className="text-[11px] uppercase tracking-[0.14em] text-slate-500">
                                Step {index + 1}
                            </div>
                            <span
                                className={`inline-flex max-w-full rounded-full border px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] ${stageToneClasses[stage.state]}`}
                            >
